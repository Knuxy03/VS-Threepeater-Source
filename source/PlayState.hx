	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				{
					case 1:
						ready = new FlxSprite().loadGraphic(Paths.image('Ready...'));
						...
						FlxTween.tween(ready.scale, {x: 30, y: 30}, 200, {type: ONESHOT});

						FlxTween.tween(ready, {y: ready.y += 15, x: ready.x += 60}, Conductor.crochet / 1000, {
						...
						FlxG.sound.play(Paths.sound('bash'));

					case 2:
						set = new FlxSprite().loadGraphic(Paths.image('Set...'));
						...
						FlxTween.tween(set.scale, {x: 30, y: 30}, 200, {type: ONESHOT});

						FlxTween.tween(set, {y: set.y += 0, x: set.x += 0}, Conductor.crochet / 1000, {
							ease: FlxEase.cubeInOut,
							onComplete: function(twn:FlxTween)
							{
								set.destroy();
					...
					case 3:
						go = new FlxSprite().loadGraphic(Paths.image('RAP!'));
						go.scrollFactor.set();
						go.updateHitbox();
						go.screenCenter();
						go.scale.x = 0.70;
						...
						FlxTween.tween(go, {y: go.y += 20}, Conductor.crochet / 1000, {
							ease: FlxEase.cubeInOut,
							onComplete: function(twn:FlxTween)
							{
								go.destroy();
